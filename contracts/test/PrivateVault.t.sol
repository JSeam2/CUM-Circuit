...
contract PrivateVaultTest is Test {
    ...
    function testDeposit_SingleCommitment() public {
        ...
        uint256 oldDepositCount = vault.getDepositCount(ETH_TOKEN);
        ...
        // Check that deposit count incremented
        uint256 newDepositCount = vault.getDepositCount(ETH_TOKEN);
        assertEq(newDepositCount, oldDepositCount + 1, "Deposit count should increment");
    ...
    function testDeposit_MultipleCommitments() public {
        ...
        for (uint256 i = 0; i < commitments.length; i++) {
            ...
            // Check deposit count
            assertEq(vault.getDepositCount(ETH_TOKEN), i + 1, "Deposit count should increment correctly");
...
contract PrivateVaultWithRealVerifierTest is Test {
    ...
    function testWithdraw_WithRealVerifier() public {
        ...
        assertTrue(vault.getDepositCount(ETH_TOKEN) == 0, "Initial Deposit Count Should Be 0");
        assertTrue(vault.getActiveDepositCount(ETH_TOKEN) == 0, "Active Deposit Count should be 0");

        // Deposit the commitment first
        vault.privateDeposit(ETH_TOKEN, commitment);

        assertTrue(vault.getDepositCount(ETH_TOKEN) == 1, "Deposit Count Should Be 1");
        assertTrue(vault.getActiveDepositCount(ETH_TOKEN) == 1, "Active Deposit Count should be 1");
        ...
        assertTrue(vault.getWithdrawalCount(ETH_TOKEN) == 0, "Withdrawal count should be zero before withdrawal");
        assertTrue(vault.getActiveDepositCount(ETH_TOKEN) == 1, "Active Deposit Count should be 1");
        ...
        assertTrue(vault.getWithdrawalCount(ETH_TOKEN) == 1, "Withdrawal count should be 1");
        assertTrue(vault.getActiveDepositCount(ETH_TOKEN) == 0, "Active Deposit Count should be 0");
